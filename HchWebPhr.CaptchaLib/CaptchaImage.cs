...
namespace HchWebPhr.CaptchaLib
{
	public class CaptchaImage : ICaptchaImage, ICaptchaValue
	{
		...
		public int CaptchaLength
		{
			get
			{
				return captchaLength > 4 ? captchaLength : 4;
			}
		...
		{
			var digits = new string(Enumerable.Repeat(Enumerable.Range(0, 10), CaptchaLength).SelectMany(v => v)
				 .OrderBy(v => Guid.NewGuid()).Take(CaptchaLength).Select(v => v.ToString(CultureInfo.InvariantCulture)[0]).ToArray());
			return digits;
		}
		...
		Bitmap PrintChar(string s)
		{
			...
			var g = Graphics.FromImage(b);
			var f = new Font(new Font(FontFamily, FontSize), FontStyle.Bold);
